x = int(100)
print("Number is " + str(x))

Number1 = "200"
Number2 = '10'
print(int(Number1) + int(Number2))

name = input("Enter Full Name: ")
email = input("Enter Email: ")
print("Name: " + name)
print("Email: " + email)

# string Place Holders {}
name = "Mc Alexis"
food = "Chocolate"
game = "Playstation"

sampleText1 = "My name is {} i love {} and playing {}"
sampleText1a = sampleText1.format(name, food, game)
print(sampleText1a)

sampleText2 = "My name is {0} i love {1} and playing {2}"
sampleText2a = sampleText1.format(name, food, game)
print(sampleText2a)

sampleText3 = "My name is {newname} i love {newfood} and playing {newgame}"
sampleText3a = sampleText1.format(newname="mike", newfood="cake", newgame="PSP")
print(sampleText3a)

## stringPlaceholders %
item = "spaghetti"
cost = 35.5651534

sampleText4 = "The product %s costs %.if" % (item, cost)
print(sampleText4)

import math
grade1 = 95.68453453
grade2 = 95.15436

print(round(grade1, 1))
print(math.ceil(grade1))
print(math.ceil(grade2))
print(math.floor(grade1))
print(math.floor(grade2))
print(pow(2, 3))
print(2**3)
